...
type MapPoint = {
  id: string;
  type: MapPointType;
  top: number; // نسبة من ارتفاع الخريطة 0–100
  left: number; // نسبة من عرض الخريطة 0–100
};

// نقاط ثابتة تمثل توزيع المصادر داخل خريطة الرياض (شكل تجريبي)
...
/**
 * دالة مساعدة لجلب JSON بأمان بدون كسر الصفحة
 */
async function fetchJson<T>(url: string): Promise<T | null> {
  try {
    ...
    console.error("NEOMIND dashboard: fetch failed", url, err);
    return null;
  }
}
...
export default async function MinisterDashboardPage() {
  // جلب البيانات من الخدمات مباشرة (بدون الـ Gateway مؤقتاً)
  const [coreHealth, psicHealth, mlHealth, heatmap] = await Promise.all([
  ...
  return (
    <div className="nm-shell nm-shell-dashboard">
      ...
      <main className="nm-grid">
        {/* العمود الأيسر: التنبؤات والتحليلات */}
        ...
        {/* العمود الأوسط: الخريطة الوطنية للمخاطر */}
        <section className="nm-column nm-column-center">
          ...
          <div className="nm-map-wrapper">
            {/* خريطة الرياض (Static tile من OpenStreetMap) */}
            <div className="nm-map nm-map-riyadh">
              {/* نقاط المصادر */}
        ...
        {/* العمود الأيمن: مخرجات الطبقات السيادية */}
        <section className="nm-column nm-column-right">
          ...
          <div className="nm-layers-list">
            ...
            <button className="nm-layer-pill">
              ...
              <span className="nm-layer-pill-sub">
                يحوّل الأحداث المتدفقة إلى مؤشرات مخاطر مكانية وزمانية،
                ويولّد خريطة حرارية للمخاطر في الرياض.
              </span>
            </button>
            ...
            <button className="nm-layer-pill">
              ...
              <span className="nm-layer-pill-sub">
                تحوّل المخرجات التحليلية إلى أوامر تشغيلية قابلة للمتابعة،
                مع تتبع حالة التنفيذ عبر التوأم الرقمي.
              </span>
            </button>
